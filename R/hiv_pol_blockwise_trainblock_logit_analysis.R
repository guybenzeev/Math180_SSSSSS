...
#   - Use that block as TRAIN
#   - Use all other codons as TEST
#   - Oversample minority class in TRAIN
#   - Fit logistic regression
#   - Tune threshold to maximize F1 on the TRAIN block
#   - Evaluate on the REST of the sequence using that threshold
...
############################################################
## 5. Loop over blocks: TRAIN on block, TEST on rest ----
############################################################
...
for (i in seq_len(nrow(blocks))) {
  ...
  cat("\n==============================\n")
  cat("Block", i, ": codons [", b_start, ", ", b_end, "] as TRAIN\n", sep = "")
  cat("==============================\n")
  
  is_train <- data_model$codon_index >= b_start & data_model$codon_index <= b_end
  ...
  # Need at least one positive in TRAIN to learn anything
  if (!("1" %in% names(train_tab)) || train_tab["1"] == 0) {
    warning("No positive examples in TRAIN for block ", i,
            " – skipping this block.")
    next
  }
  # Need at least one positive in TEST for meaningful recall/F1
  ...
  ########################################################
  # 5a. Oversample minority in TRAIN block
  ########################################################
  
  train_os <- oversample_minority(train_df)
  
  # Class weights (after oversampling; often ~balanced)
  ...
  ########################################################
  # 5b. Fit logistic regression on TRAIN block
  ########################################################
  ...
  ########################################################
  # 5c. Tune threshold using TRAIN block performance
  #     (use predictions on ORIGINAL train_df, not oversampled)
  ########################################################
  
  y_train_true <- train_df$resistance_site
  train_probs  <- as.numeric(predict(logit_fit, newdata = train_df, type = "response"))
  
  best <- tune_threshold_pr(y_train_true, train_probs, n_grid = 300)
  cat("Best threshold (based on TRAIN block) for block", i, ":\n")
  print(best)
  
  ########################################################
  # 5d. Evaluate on TEST (rest of pol)
  ########################################################
  
  y_test_true <- test_df$resistance_site
  test_probs  <- as.numeric(predict(logit_fit, newdata = test_df, type = "response"))
  
  metrics <- evaluate_binary(y_test_true, test_probs, threshold = best$threshold)
  ...
  results_list[[i]] <- data.frame(
    ...
    train_neg     = ifelse("0" %in% names(train_tab), as.numeric(train_tab["0"]), 0),
...
cat("\n\n===== Train-Block / Test-Rest Logistic Regression Results =====\n")
